...
class ViewTableCopaBrasil extends Component {
  ...
  componentDidMount = async () => {
    ...
    const { data: getNextStepCopaBr } = await Apiresponse.getNextStepCopaBr(2, getRound.fase_atual.fase_id)
    this.setState({
      time: [],
      artilharia: getArtilheiro.slice(0, 10),
      rodada: getNextStepCopaBr.chaves,
    })
  }
    ...
    return this.state.rodada.map((element) => {
      ...
      return (
        <tr key={element.fase_id}>
          ...
          <td>{element.partida_ida.hora_realizacao} - {element.partida_ida.data_realizacao}</td>
        </tr>
      )
    })
  }
